# -*- coding: utf-8 -*-
"""
Created on Fri Apr 23 13:04:29 2021
...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr 16 08:14:39 2021
...
#script for generating a single image with three panes of SDV plots
    # for each model pair
#         -temperature wise
# 			-For T90:
# 			1) SDV GCM
...
def generate_Single_SDV_Plots():
    t_Data_Types = ["tasmax90"]
    p_Data_Types = ["pr95"]
    wrf_Folder = r"Netcdf_Files" + "\\" +"wrf_Netcdf_Files"
    gcm_Folder = r"Netcdf_Files" + "\\" +"gcm_Netcdf_Files"
    #for each wrf and gcm model pair, generate SDV plot for each of the sampled data types
            ...
            if(gcm_tail.__str__().find(wrf_Model)>=0):
                ...
                if(wrf_tail.__str__().find("T2MAX")>=0 and gcm_tail.__str__().find("tasmax")>=0):
                    for temperature_Type in t_Data_Types:
                        ...
                        gcm_SDV = gcm_Data.variables["Interpolated SDV data based on " + temperature_Type][:]
                        wrf_lon = wrf_Data.variables["XLONG"][:] 
                        wrf_lat = wrf_Data.variables["XLAT"][:]
                        wrf_SDV = wrf_SDV[4:-4,4:-4]
                        wrf_lon = wrf_lon[4:-4,4:-4]
                        wrf_lat = wrf_lat[4:-4,4:-4]
                        gcm_SDV = gcm_SDV[4:-4,4:-4]
                       
                        plt.figure(figsize=(7.5,5))
                        WRFplot(wrf_SDV, wrf_lat, wrf_lon,  amin(wrf_SDV),amax(wrf_SDV), "SDV for "  + wrf_File.split("\\")[2].split("-")[0] + "-wrf" 
                        ...
                        plt.figure(figsize=(7.5,5))
                        WRFplot(gcm_SDV, wrf_lat, wrf_lon,  amin(gcm_SDV),amax(gcm_SDV), "SDV for "  + gcm_File.split("\\")[2].split("_")[0] + "-gcm" 
                ...
                if(wrf_tail.__str__().find("PREC")>=0 and gcm_tail.__str__().find("pr_extr")>=0):
                    for precipitation_Type in p_Data_Types:
                        ...
                        gcm_Data = Dataset(gcm_File, "r" , format = "NETCDF4") 
                        wrf_SDV = wrf_Data.variables["Standard_Deviations_for_" + wrf_Data_Type][:]
                        gcm_SDV = gcm_Data.variables["Interpolated SDV data based on " + precipitation_Type][:]
                        wrf_lon = wrf_Data.variables["XLONG"][:] 
                        wrf_lat = wrf_Data.variables["XLAT"][:]
                        wrf_SDV = wrf_SDV[4:-4,4:-4]
                        wrf_lon = wrf_lon[4:-4,4:-4]
