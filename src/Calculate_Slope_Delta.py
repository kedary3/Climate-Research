# -*- coding: utf-8 -*-
"""
Created on Wed May  5 01:48:43 2021
...
# -*- coding: utf-8 -*-
"""
Created on Tue May  4 22:06:56 2021
...
#Delta Slope for access and ccsm4 between wrf and gcm for temp and try prec
		#all combinations
from numpy import (
...
def Calculate_Slope_Delta(wrf_File, gcm_File, delta_File, gcm_Data_Type):
    wrf_Data_Type = " "
    ...
    gcm_Slope = gcm_Data.variables["Interpolated Slope data based on " + gcm_Data_Type][:]
    ...
    ny = shape(wrf_Slope)[0] # south-north - 123 - wrf / south-north - 21 - gcm
    nx = shape(wrf_Slope)[1] # west-east - 162 - wrf / west-east - 33 - gcm
    for k in range(ny):
        for i in range(nx):
            
            d = wrf_Slope[[k],[i]]-gcm_Slope[[k],[i]]
            deltas[[k],[i]]=d
    ...
    WRFplot(deltas, wrf_lat, wrf_lon,  amin(wrf_Slope),amax(wrf_Slope), "Delta between "  + wrf_File.split("\\")[2].split("-")[0] + "-wrf" +
                                      " and " + gcm_File.split("\\")[2].split("_")[0] + "-gcm" + " based on " + gcm_Data_Type ,"Difference in Slope", "RdYlBu_r")
    
    plt.savefig("Slope Deltas" + "\\" + "Delta between "  + wrf_File.split("\\")[2].split("-")[0] + "-wrf" +
...
if(os.path.exists("Netcdf_Files" + "\\" + "Slope_Deltas_from_gcm_to_wrf.nc") == False):
    delta_File = "Netcdf_Files" + "\\" + "Slope_Deltas_from_gcm_to_wrf.nc"
    ds=clone("Netcdf_Files" + "\\" + "wrf_grid.nc", delta_File)

#for each wrf and gcm file, generate Slope data for each data type and get interpolated gcm data
delta_File = "Netcdf_Files" + "\\" + "Slope_Deltas_from_gcm_to_wrf.nc"
# (wrf, gcm) (data_Type)
...
#for each wrf and gcm file, generate Slope delta data for each data type
        ...
        if(gcm_tail.__str__().find(wrf_Model)>=0):
            ...
            if(wrf_tail.__str__().find("T2MAX")>=0 and gcm_tail.__str__().find("tasmax")>=0):
                for temperature_Type in t_Data_Types:
                    Calculate_Slope_Delta("Netcdf_Files" + "\\" + "wrf_Netcdf_Files" + "\\" + wrf_tail, "Netcdf_Files" + "\\" + "gcm_Netcdf_Files" + "\\" +  gcm_tail, delta_File, temperature_Type)  
            if(wrf_tail.__str__().find("PREC")>=0 and gcm_tail.__str__().find("pr_extr")>=0):
                for precipitation_Type in p_Data_Types:
                    Calculate_Slope_Delta("Netcdf_Files" + "\\" + "wrf_Netcdf_Files" + "\\" + wrf_tail, "Netcdf_Files" + "\\" + "gcm_Netcdf_Files" + "\\" +  gcm_tail, delta_File, precipitation_Type)  
