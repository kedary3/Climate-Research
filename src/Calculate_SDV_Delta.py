...
#Delta SDV for access and ccsm4 between wrf and gcm for temp and try prec
		#all combinations
from numpy import (
...
def Calculate_SDV_Delta(wrf_File, gcm_File, delta_File, gcm_Data_Type):
    wrf_Data_Type = " "
    ...
    wrf_SDV = wrf_Data.variables["Standard_Deviations_for_" + wrf_Data_Type][:]
    gcm_SDV = gcm_Data.variables["Interpolated SDV data based on " + gcm_Data_Type][:]
    ...
    ny = shape(wrf_SDV)[0] # south-north - 123 - wrf / south-north - 21 - gcm
    nx = shape(wrf_SDV)[1] # west-east - 162 - wrf / west-east - 33 - gcm
    for k in range(ny):
        for i in range(nx):
            
            d = wrf_SDV[[k],[i]]-gcm_SDV[[k],[i]]
            deltas[[k],[i]]=d
    ...
    WRFplot(deltas, wrf_lat, wrf_lon,  amin(wrf_SDV),amax(wrf_SDV), "Delta between "  + wrf_File.split("\\")[2].split("-")[0] + "-wrf" +
...
if(os.path.exists("Netcdf_Files" + "\\" + "SDV_Deltas_from_gcm_to_wrf.nc") == False):
    delta_File = "Netcdf_Files" + "\\" + "SDV_Deltas_from_gcm_to_wrf.nc"
    ds=clone("Netcdf_Files" + "\\" + "wrf_grid.nc", delta_File)

#for each wrf and gcm file, generate SDV data for each data type and get interpolated gcm data
delta_File = "Netcdf_Files" + "\\" + "SDV_Deltas_from_gcm_to_wrf.nc"
# (wrf, gcm) (data_Type)
...
#for each wrf and gcm file, generate SDV delta data for each data type
        ...
        if(gcm_tail.__str__().find(wrf_Model)>=0):
            ...
            if(wrf_tail.__str__().find("T2MAX")>=0 and gcm_tail.__str__().find("tasmax")>=0):
                for temperature_Type in t_Data_Types:
                    Calculate_SDV_Delta("Netcdf_Files" + "\\" + "wrf_Netcdf_Files" + "\\" + wrf_tail, "Netcdf_Files" + "\\" + "gcm_Netcdf_Files" + "\\" +  gcm_tail, delta_File, temperature_Type)  
            if(wrf_tail.__str__().find("PREC")>=0 and gcm_tail.__str__().find("pr_extr")>=0):
                for precipitation_Type in p_Data_Types:
                    Calculate_SDV_Delta("Netcdf_Files" + "\\" + "wrf_Netcdf_Files" + "\\" + wrf_tail, "Netcdf_Files" + "\\" + "gcm_Netcdf_Files" + "\\" +  gcm_tail, delta_File, precipitation_Type)  
